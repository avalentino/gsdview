...
class ProcessTestCase(unittest.TestCase):
    ...
    def test_stdin_fileobj(self):
        ...
        p = subprocess.Popen(
            [sys.executable,
             "-c", 'import sys; sys.exit(sys.stdin.read() == "pear")'],
            stdin=tf)
        p.wait()
        self.assertEqual(p.returncode, 1)
    ...
    def test_no_leaking(self):
        ...
        if (not hasattr(test_support, "is_resource_enabled") or
                test_support.is_resource_enabled("subprocess") and
                not mswindows):
...
if __name__ == "__main__":
    if not hasattr(subprocess, "check_call"):
        del ProcessTestCase.test_check_call_zero
        del ProcessTestCase.test_check_call_nonzero
